...
const updateTrendingNames = () => { const now = new Date(); const weekSeedStr = `${now.getFullYear()}-${getWeekNumber(now)}`; const seed = stringToSeed(weekSeedStr); const random = mulberry32(seed); const shuffled = [...names].sort(() => random() - 0.5); const trendingNames = shuffled.slice(0, 3); const statuses = [ { class: 'rising', icon: 'fa-arrow-trend-up', popKey: "popularityRising" }, { class: 'top-pick', icon: 'fa-star', popKey: "popularityTopPick" }, { class: 'popular', icon: 'fa-fire', popKey: "popularityPopular" } ]; trendingContainer.innerHTML = ''; const currentLang = localStorage.getItem('language') || 'en'; trendingNames.forEach((name, index) => { const status = statuses[index]; const tagKey = `tag${status.class.charAt(0).toUpperCase() + status.class.slice(1).replace('-p', 'P')}`; const cardHTML = `<div class="trending-card ${status.class}"> <div class="icon-status"><i class="fa-solid ${status.icon}"></i></div> <h4 class="name">${name.name}</h4> <span class="tag tag-${status.class}" data-i18n-key="${tagKey}">${translations[currentLang][tagKey]}</span> <p class="popularity" data-i18n-key="${status.popKey}">${translations[currentLang][status.popKey]}</p> </div>`; trendingContainer.innerHTML += cardHTML; }); };
